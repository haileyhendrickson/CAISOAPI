...
def backend(market_run_id, startdate, enddate): 
    ...
    def pull_request(startdate, enddate): # 
        ...
        params = {
            ...
            "queryname": queryname,  # Queryname represents the report name, based on market_run_id
            "startdatetime": f'{startdate}T07:00-0000',  # Dates start at the 7 hour for MST purposes
                ...
                for filename in z.namelist():
                    with z.open(filename) as f:
                        ...
                        if '<?xml version="1.0" encoding="UTF-8"?>' in df.columns:
                            pass  # If this error is found, the logic doesn't try to add in these files that are empty
                        else:  # This only appends files that pull correctly
                            files.append(f'pull#{counter}.csv')  # Creating a list of all files I need to combine
        except Exception as e:  # There are typcially not errors with pulling the file, so the above error handling catches parameter errors
    ...
    def cleanFile(filename):
        ...
        df = df.drop(columns=valid_columns).sort_values(['INTERVALSTARTTIME_GMT'])  # Dropping columns conditionally, so if they are in the df, they are dropped. 

        # Changing the timezone to MST
        df['INTERVALSTARTTIME_GMT'] = df['INTERVALSTARTTIME_GMT'].str.replace('-00:00', '').str.replace('T', ' ') 
        df['INTERVALENDTIME_GMT'] = df['INTERVALENDTIME_GMT'].str.replace('-00:00','').str.replace('T',' ') 
        df['INTERVALSTARTTIME_GMT'] = pd.to_datetime(df['INTERVALSTARTTIME_GMT'])  # Changing to datetime datatype
        df['INTERVALSTARTTIME_GMT'] = df['INTERVALSTARTTIME_GMT'] - timedelta(hours=7)  # Shifting the hours back by 7 to align with MST
    ...
    def fill_missing_values(filename, market_run_id):
        ...
        df['INTERVALSTARTTIME_MST'] = pd.to_datetime(df['INTERVALSTARTTIME_MST']).dt.floor(f'{interval_minutes}min')  # Makes it some sort of set so I can subtact it later
        full_range = pd.date_range(start=df['INTERVALSTARTTIME_MST'].min(), end=df['INTERVALSTARTTIME_MST'].max(), freq=f'{interval_minutes}min')  # Finding all intervals I should have 
        full_df = pd.DataFrame({'INTERVALSTARTTIME_MST': full_range})  # New df for all intervals I need
        ...
        interval_end = result['INTERVALSTARTTIME_MST'] + pd.Timedelta(minutes=interval_minutes)  # Finding starttime and adding 60 minutes
        result['INTERVALENDTIME_MST'] = result['INTERVALENDTIME_MST'].fillna(interval_end)  # Filling in empty values For interval end
        result = result.sort_values(['INTERVALSTARTTIME_MST', 'NODE'])  # Sorting by node so I can backfill
        ...
        # Filling in LMP values, backfilling by taking previous day's same hour and minute intvl price.
    ...
    def hourly_average(filename):
        ...
        if 'Greenhouse Gas' in df.columns: 
            ...
            df_avg = df.groupby(['NODE', 'Year', 'Month', 'Day', 'Hour (MST)'], as_index=False)[['Congestion', 'Energy', 'LMP', 'Loss']].mean() 
            df_avg[['Congestion', 'Energy', 'Loss','LMP']] = df_avg[['Congestion', 'Energy', 'Loss', 'LMP']].round(4)
            df_avg = df_avg[['NODE', 'Year', 'Month', 'Day', 'Hour (MST)', 'Congestion', 'Energy', 'Loss', 'LMP']]
        ...
        with pd.ExcelWriter(f'{output_file_path}/{market_run_id} {timestamp}.xlsx', engine='openpyxl', mode='a', if_sheet_exists='overlay') as writer:
            ...
            pd.DataFrame([['Number of hours LMP is below 0:']]).to_excel(writer, sheet_name='Summary Statistics', startrow=row, header=False, index=False)  # Adding title to sheet
            row += 1
            pd.DataFrame([[count]]).to_excel(writer, sheet_name='Summary Statistics', startrow=row, header=False, index=False)  # Adding summary stats to sheet
            row += 2 
            pd.DataFrame([['Duration Curve']]).to_excel(writer, sheet_name='Summary Statistics', startrow=row, header=False, index=False)
            row += 1

        # Heat map logic
        pivot = df.pivot_table(index='Hour (MST)', columns='Month', values='LMP', aggfunc='mean')  # Creating a new df for heatmap
        plt.figure(figsize=(10,8)) 
    ...
    def summary_statistics(filename): 
        ...
        with pd.ExcelWriter(f'{output_file_path}/{market_run_id} {timestamp}.xlsx', engine='openpyxl', mode='a', if_sheet_exists='overlay') as writer:  # Adding new sheet to excel file
            row = 0
            pd.DataFrame([['Summary Statistics']]).to_excel(writer, sheet_name='Summary Statistics', startrow=row, header=False, index=False)  # Adding title to sheet
            row += 1
    ...
    def duration_chart(filename): 
        ...
        total_count = df['LMP'].value_counts().sum()  # Counting total values
        df['duration_count'] = df['LMP'].map(duration_counts)  # This represents how many there are of that specific value
        df['percent'] = df['duration_count']/total_count  # Percentage of how often that value appears in the df
    ...
    def get_ordered_columns(has_greenhouse_gas=True):
        base_columns = ['INTERVALSTARTTIME_MST', 'INTERVALENDTIME_MST', 'NODE', 'Year', 'Month', 'Day', 'Hour (MST)', 'Minute']
    ...
    for file in files:  # If tabbed over, it cleans each pull before combining, but is bad at error handling
        cleanFile(file) 
        df_list.append(pd.read_csv(file))  # Adding rows in the csv to df_list for later concatenation
    ...
    if value_column in df_combined.columns:
        df_combined = pd.pivot_table(df_combined, values=value_column, index=['INTERVALSTARTTIME_MST', 'INTERVALENDTIME_MST', 'NODE', 'Year', 'Month', 'Day', 'Hour (MST)', 'Minute'], columns='LMP_TYPE').reset_index()  # Breaking out LMP_TYPE columns, keeping the other indexed columns
...
def findStartDate():  # Creates start date based on user input
    global startdate
    startdate = cal.get_date()
    startdate_label.configure(text=f'Start date: {startdate}')

def findEndDate():  # Creates end date based on user input
    global enddate 
    enddate = cal.get_date()
    enddate_label.configure(text=f'End date: {enddate}')

def select_output_file():  # Allows user to select an output location and creates a path
    ...
    if directory:
        ...
        output_file_label.configure(text='No directory selected yet')  # If submitted without a filepath
